...
def rep_clientes_con_mas_pedidos():
    ...
    commit = listar_clientes_con_mas_pedidos(limit=limit)
    return jsonify(commit), 200

def rep_usuarios_con_mas_pedidos_registrados():
    limit = request.args.get("limit", default=5, type=int)
    commit = listar_usuarios_con_mas_registro_pedidos(limit=limit)
    return jsonify(commit), 200
...
def rep_productos_mas_vendidos():
    ...
    commit = listar_productos_mas_vendidos(desde, hasta, limite)
    if commit:
...
def rep_pedidos_por_fecha():
    ...
    commit = listar_pedidos_por_fecha(desde, hasta, estado)
    if commit:
...
def rep_productos_mas_ganancias():
    ...
    commit = listar_productos_mas_ganancias(desde, hasta, limite)
    if commit:
        return jsonify({"mensaje": "Producto encontrado exitosamente",
...
def rep_ingresos_generados():
    ...
    commit = listar_ingresos_por_ventas(desde, hasta)
    if commit:

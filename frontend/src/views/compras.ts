...
function renderPurchaseCart() {
    ...
    tbody.innerHTML = purchaseCart.map((item, index) => {
        if (!item.solo_stock) total += item.costo * item.cantidad;
            ...
            <tr class="compra-row ${item.solo_stock ? 'compra-row--solo-stock' : ''}">
                ...
                <td class="text-center">
                    <label class="compra-checkbox-label">
                        ...
                        <span class="text-nowrap">Ya existente</span>
                    </label>
                </td>
                <td class="text-center">
...
function hasDuplicates(): string | null {
    ...
    for (const n of names) { if (seen.has(n)) return n; seen.add(n); }
    return null;
}
...
function addExistingProduct(product: Product & { stock: number }) {
    ...
    if (!alreadyExists) {
        purchaseCart.push({ ref: product.ref, nombre: product.nombre, costo: Number(product.costo_promedio), precio_venta: Number(product.precio_venta), ref_categoria: product.ref_categoria, cantidad: 1, solo_stock: false });
    } else { alreadyExists.cantidad += 1; }
}

function addNewProduct(baseName: string) {
    if (purchaseCart.some(c => c.nombre.trim().toLowerCase() === baseName.trim().toLowerCase())) return;
    purchaseCart.push({ nombre: baseName || 'Nuevo Producto', costo: 0, precio_venta: 0, cantidad: 1, solo_stock: false });
}

export async function renderPurchases(container: HTMLElement) {
    purchaseCart = []; globalInventory = []; globalCategories = [];
    container.innerHTML = '<h2 class="loading-state">Cargando inventario...</h2>';
    try {
        const [products, inventories, categories] = await Promise.all([api.getProducts(), api.getInventories(), api.getCategories()]);
        globalCategories = categories;
        const stockMap: Record<string, number> = {};
        inventories.forEach(inv => stockMap[inv.ref_producto] = inv.cantidad_actual);
        globalInventory = products.map(p => ({ ...p, stock: stockMap[p.ref] ?? 0, costo_promedio: Number(p.costo_promedio || 0), precio_venta: Number(p.precio_venta || 0) }));
        ...
        dropdown.addEventListener('click', (e) => {
            ...
            if (product) { addExistingProduct(product); searchInput.value = ''; dropdown.style.display = 'none'; searchInput.focus(); updateVisibility(); renderPurchaseCart(); }
        });
        ...
        searchInput.addEventListener('keydown', (e) => {
            if (e.key === 'Enter') {
                ...
                if (e.shiftKey) {
                    ...
                    if (existsInCart || existsInInventory) { showFeedback('error', `Ya existe un producto con el nombre "${text}"`); return; }
                    addNewProduct(text);
                } else {
                ...
                searchInput.value = ''; dropdown.style.display = 'none'; searchInput.focus(); updateVisibility(); renderPurchaseCart();
            }
        });

        document.addEventListener('click', (e) => { if (!(e.target as HTMLElement).closest('.pos-search-wrapper')) dropdown.style.display = 'none'; });
        ...
        document.getElementById('tabla-compras')!.addEventListener('change', (e) => {
            ...
            if (target.classList.contains('pos-input-edit')) {
                ...
                if (field === 'cantidad' || field === 'costo' || field === 'precio_venta') { (purchaseCart[i] as any)[field] = parseFloat(target.value) || 0; }
                    ...
                    if (dupInCart || dupInInventory) { showFeedback('error', `Ya existe un producto con ese nombre`); target.value = purchaseCart[i].nombre; return; }
                    purchaseCart[i].nombre = target.value;
                } else { (purchaseCart[i] as any)[field] = target.value; }
                renderPurchaseCart();
            } else if (target.classList.contains('pos-input-checkbox')) { purchaseCart[i].solo_stock = (target as HTMLInputElement).checked; renderPurchaseCart(); }
        });

        document.getElementById('tabla-compras')!.addEventListener('click', (e) => {
            const btn = (e.target as HTMLElement).closest('.btn-eliminar-compra');
            if (btn) { purchaseCart.splice(parseInt(btn.getAttribute('data-index') || '0'), 1); updateVisibility(); renderPurchaseCart(); }
        });
        ...
        document.getElementById('btn-registrar-compra')!.addEventListener('click', async () => {
            ...
            if (dup) { showFeedback('error', `Hay nombres duplicados en la lista: "${dup}"`); return; }
            ...
            try {
                const payload = purchaseCart.map(c => ({ ref_producto: c.ref || null, nombre: c.nombre, costo: c.costo, precio_venta: c.precio_venta, ref_categoria: c.ref_categoria, cantidad: c.cantidad, solo_stock: c.solo_stock }));
                const result: any = await api.registerQuickPurchase(payload);
                purchaseCart = []; updateVisibility(); renderPurchaseCart();
                await reloadInventorySilent();
                showFeedback('success', `${result.mensaje} — Total: $${(result.compra?.total ?? 0).toFixed(2)}`);
            } catch (err: any) { showFeedback('error', err.message || 'Error al registrar');
            } finally {
        ...
        async function reloadInventorySilent() {
            ...
            globalInventory = prods.map(p => ({ ...p, stock: stkMap[p.ref] ?? 0, costo_promedio: Number(p.costo_promedio || 0), precio_venta: Number(p.precio_venta || 0) }));
        }
    } catch (e) {
...
function updateVisibility() {
    ...
    if (purchaseCart.length > 0) { if (table) table.style.display = 'table'; if (empty) empty.style.display = 'none'; }
    else { if (table) table.style.display = 'none'; if (empty) empty.style.display = ''; }
}
